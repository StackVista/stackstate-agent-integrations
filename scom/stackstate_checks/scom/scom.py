...
def hasChild(serviceid,session, scom_ip, domain, username, password):
        response = (session.get('http://' + scom_ip + '/OperationsManager/data/objectInformation/' + serviceid,auth=HttpNtlmAuth(domain + '\\' + username, password))).json()
...
class SCOM(AgentCheck):
    ...
    def sendAlerts(self,session, scom_ip, domain, username, password,component_id):
        data = {
            ...
            "displayColumns":
            [
                "id", "name", "monitoringobjectdisplayname", "description", "resolutionstate", "timeadded","monitoringobjectpath"
            ]
        }
    ...
    def getComponentDetails(self,id,session, scom_ip, domain, username, password):
        response = (session.get('http://' + scom_ip + '/OperationsManager/data/objectInformation/' + id,auth=HttpNtlmAuth(domain + '\\' + username, password))).json()
    ...
    def nextNodes(self,serviceid,session, scom_ip, domain, username, password,system_type):
        ...
        for idd in data:
            ...
            self.event({
                ...
                "tags": [
                    "status:%s" % get_entity_health_state(session, scom_ip, domain, username, password,idd.get("healthIconUrl")),
                    "id:%s" % idd.get("id")
                ]
            })
    ...
    def serviceTree(self,serviceidtree,session, scom_ip, domain, username, password,system_type):
        for serviceid in serviceidtree:
            if not hasChild(serviceid,session, scom_ip, domain, username, password):
                ...
                self.serviceTree(self.nextNodes(serviceid,session, scom_ip, domain, username, password,system_type),session, scom_ip, domain, username, password,system_type)
    def check(self, instance):
        ...
        session = Session()
        self.log.info('Connection Status Code ' + init_session(session,auth_method, domain, username, password, scom_ip))
        ...
        try:
            ...
            for stream in streams:
                ...
                for component in root_tree:
                    ...
                    health = session.get('http://' + scom_ip + '/OperationsManager/data/healthstate/' + component.get("id", None), auth=HttpNtlmAuth(domain + '\\' + username, password))
                    self.event({
                        "timestamp": int(time.time()),
                        "msg_title": "Health Status",
                        "msg_text": "Change in health status",
                        ...
                        "tags": [
                            "status:%s" % get_entity_health_state(session, scom_ip, domain, username, password, health.text ),
