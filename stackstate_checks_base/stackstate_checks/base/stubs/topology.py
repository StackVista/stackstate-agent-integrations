...
class TopologyStub(object):
    ...
    def _is_duplicate_relation(self, check_id, instance_key, source_id, target_id, type, data):
        if relation(source_id, target_id, type, data) in self._ensure_instance(check_id, instance_key)["relations"]:
            return True
        return False

    def _is_duplicate_component(self, check_id, instance_key, id, type, data):
        if component(id, type, data) in self._ensure_instance(check_id, instance_key)["components"]:
            return True
        return False
