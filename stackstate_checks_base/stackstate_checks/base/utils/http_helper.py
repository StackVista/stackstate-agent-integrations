...
class HTTPHelperRequestHandler:
    ...
    def get_request(self):
        self.log.info("Retrieve the active Request() object.")
        return self._request

    # Allows the developer to use a custom request object if required.
    # This opens up some flexibility and control
    def set_request(self, request):
        self.log.info("Replacing the old Request() object with a user defined one.")
        if isinstance(request, type(Request())):
            self._request = request
        else:
            message = """The request object passed to this function is the incorrect type.
    ...
    def reset_request(self):
        self.log.info("Creating a new Request() object and replacing the old one.")
        self._request = Request()
    ...
    def set_method(self, method):
        ...
        if isinstance(method, Enum) and \
                ...
                method.value in self.http_method_enums:
        ...
        elif isinstance(method, str) and method in self.http_method_enums:
    ...
    def set_url(self, url):
        ...
        self.log.info("Attempting to set the active current url to {0}".format(str(url)))
    ...
    def get_query_param(self):
        self.log.info("Retrieving current HTTP Query Parameters from the Request() object")
        return self._request.params

    # Reset the value of the current Query Parameters on the Request() object
    def clear_query_param(self):
        self.log.info("Setting the HTTP Query Parameters object to a empty {} on the Request() object")
    ...
    def set_query_param(self, parameters):
        ...
        self.log.info("Attempting to set the active query parameters to {0}".format(str(parameters)))
    ...
    def get_body(self):
        self.log.info("Retrieving current HTTP Data Parameters from the Request() object")
        return self._request.data

    # Reset the value of the current Query Parameters on the Request() object
    def clear_body(self):
        self.log.info("Clear current HTTP Data Parameters from the Request() object")
        self._request.data = []
    ...
    def get_headers(self):
        self.log.info("Retrieving current HTTP Headers from the Request() object")
        return self._request.headers

    # Reset the value of the current Headers on the Request() object
    def clear_headers(self):
        self.log.info("Clearing the active HTTP Headers from the Request() object")
    ...
    def set_headers(self, headers):
        ...
        """
        self.log.info("Attempting to set the headers to {0}".format(str(headers)))
    ...
    def get_auth(self):
        self.log.info("Retrieving current Authentication from the Request() object")
        return self._request.auth

    # Reset the value of the current Authentication on the Request() object
    def clear_auth(self):
        self.log.info("Clearing the active Authentication from the Request() object")
    ...
    def set_auth(self, auth_schematic, auth_details):
        ...
        if isinstance(auth_schematic, Enum) and \
                auth_schematic.value in self.authentication_types and \
            ...
            try:
                ...
                message = """The authentication supplied {0} does not match the required
                             ...
                             {2}""".format(str(auth_details), str(auth_schematic), e)
                self.log.error(message)
                ...
                message = """The authentication details object passed to this function failed as
                             ...
                             The error provided by the execution
                             {1}""".format(type(auth_details), e)
                self.log.error(message)
                raise TypeError(message)

        else:
            message = """The `auth_schematic` variable passed to the `set_auth` function"
    ...
    def remove_body_type_validation(self):
        self.log.info("""Removing the body type validation""")
    ...
    def set_body_type_validation(self, body_type):
        ...
        if isinstance(body_type, Enum) and \
                ...
                body_type.value in self.request_types:
            # Save the validation to test against later
            self._body_type_validation = body_type
            self.log.info("""The body is valid and matched either the defined {0}
                                      type or is empty."""
            ...
            message = """The body does not conform to the body type provided ({0}).
                         ...
                         The current body tested content is {1}""".format(str(body_type), self._request.data)
            self.log.error(message)
            raise DataError(message)
    ...
    def remove_body_schematic_validation(self):
        self.log.info("""Removing the body schematic validation""")
    ...
    def set_body_schematic_validation(self, body_schematic):
        ...
        try:
            ...
            message = """The `body_schematic` variable passed to the `validate_body_structure`"
    ...
    def validate(self):
        self.log.info("Attempting to validate the request structure.")
        ...
        if self._body_type_validation is not None and \
            ...
            message = """The request was unable to conform to the type validation. The
...
class HTTPHelperSessionHandler:
    ...
    def get_session(self):
        self.log.info("Retrieve the active Session() object.")
        return self._session

    # Allows the developer to use a custom session object if required.
    # This opens up some flexibility and control
    def set_session(self, session):
        self.log.info("Replacing the old Session() object with a user defined one.")
    ...
    def reset_session(self):
        self.log.info("Creating a new Session() object and replacing the old one.")
        self._session = Session()
    ...
    def clear_headers(self):
        self.log.info("Clearing the headers on the Session() object.")
        self._session.headers.clear()

    # Retrieve the current headers object from the Session() object
    def get_headers(self):
        self.log.info("Retrieving the headers on the Session() object.")
    ...
    def set_headers(self, headers):
        ...
        self.log.info("""Applying the following headers object to the Session() object {0}"""
                         .format(str(headers)))
    ...
    def clear_auth(self):
        self.log.info("Clearing the authentication method on the Session() object.")
        self._session.auth = None

    # Retrieve the current authentication method from the Session() object
    def get_auth(self):
        self.log.info("Retrieving the authentication method on the Session() object.")
    ...
    def set_auth(self, auth_schematic, auth_details):
        ...
        Functionality:
            ...
            The model is applied by specifying a Enum that defines the authentication structure, A second parameter is
            then provided which maps the data structure into the proper requests authentication model.
...
class HTTPHelperConnectionHandler:
    ...
    def get_timeout(self):
        self.log.info("Retrieving the timeout on the connection.")
        return self._timeout

    # Remove the active timeout
    def clear_timeout(self):
        self.log.info("Clearing the timeout on the connection.")
        self._timeout = None
    ...
    def set_timeout(self, timeout):
        ...
        """
        self.log.info("Attempting to set the timeout to {0} seconds"
                         .format(str(timeout)))
    ...
    def get_retry_policy(self):
        self.log.info("Retrieving the retry policy on the connection.")
        return self._retry_policy

    # Remove the retry policy timeout
    def clear_retry_policy(self):
        self.log.info("Clearing the retry policy on the connection.")
    ...
    def clear_ssl_verify(self):
        self.log.info("Clearing the SSL Verification on the connection.")
        self._ssl_verify = True

    # Retrieve the current state of the SSL verification
    def get_ssl_verify(self):
        self.log.info("Retrieving the SSL Verification on the connection.")
    ...
    def set_ssl_verify(self, verify):
        ...
        if isinstance(verify, bool):
            ...
            message = """Unable to set the SSL Verification as the defined parameters is the
                         ...
                         expected""".format(str(type(verify)))
            self.log.error(message)
            raise TypeError(message)
    ...
    def clear_proxy(self):
        self.log.info("Clearing the proxy settings on the Session() object.")
        self._proxy = None

    def get_proxy(self):
        self.log.info("Retrieving the proxy settings on the Session() object.")
        return self._proxy
    ...
    def set_proxy(self, proxy):
        ...
        self.log.info("Attempting to set the proxy details to {0}"
                         .format(str(proxy)))
    ...
    def send(self, session_handler, request_handler, response_handler):
        ...
        self.log.info("Sending the HTTP request.")
...
class HTTPHelperResponseHandler:
    ...
    def clear_body_schematic_validation(self):
        self.log.info("Clearing the body schematic validation on the response.")
        self._validate_schematic = None

    # Retrieve the current response body validation
    def get_body_schematic_validation(self):
        self.log.info("Retrieving the body schematic validation on the response.")
    ...
    def clear_status_code_validation(self):
        self.log.info("Retrieving the status code validation on the response.")
    ...
    def set_status_code_validation(self, status_code):
        ...
        if isinstance(status_code, int):
            ...
            message = """The proxy schematic does not contain the correct type.
                         ...
                         status code against""".format(str(type(status_code)))
            self.log.error(message)
            raise TypeError(message)

    # Clear the body type validation on the response object
    def clear_body_type_validation(self):
        self.log.info("Clear the body type validation on the response.")
        self._validate_type = None

    # Retrieve the body type validation on the response object
    def get_body_type_validation(self):
        self.log.info("Retrieving the body type validation on the response.")
    ...
    def set_body_type_validation(self, response_type):
        ...
        if isinstance(response_type, Enum) and \
                response_type.value in self.response_types:
    ...
    def validate(self, response):
        ...
        if self._validate_schematic is not None:
            try:
                ...
                message = """The response was unable to conform to the validation schematic. The schematic
                             applied was {0}
                             The error provided by the schematic validation is {1}
                             To fix this you can either modify the schematic validation or remove it entirely
        ...
        # If the status code has not been set then we will by default see 400 > as bad responses.
            ...
            message = """Server responded with a {0}""".format(str(response.status_code))
            self.log.error(message)
            errors.append(message)
