...
from stackstate_checks.base import AgentCheck, KubeLeaderElectionBaseCheck
from stackstate_checks.base.checks.kube_leader import ElectionRecord
...
def mock_incluster():
    ...
    with mock.patch('stackstate_checks.base.checks.kube_leader.mixins.config.load_incluster_config'):
        yield
...
def mock_read_endpoints():
    ...
    with mock.patch(
        'stackstate_checks.base.checks.kube_leader.mixins.client.CoreV1Api.read_namespaced_endpoints',
    ) as m:
        yield m
...
def mock_read_configmap():
    ...
    with mock.patch(
        'stackstate_checks.base.checks.kube_leader.mixins.client.CoreV1Api.read_namespaced_config_map',
    ) as m:
        yield m
...
class TestElectionRecord:
    ...
    @pytest.mark.skip(reason="[BS] This test is also broken in the source repo. disabling for now.")
...
@mock.patch('stackstate_checks.base.checks.kube_leader.mixins.config')
class TestClientConfig:
    ...
    @mock.patch('stackstate_checks.base.checks.kube_leader.mixins.datadog_agent')
