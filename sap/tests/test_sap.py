            ...
            components=[
                {"id": "urn:host:/LAB-SAP-001",
                 ...
                 "data": {"host": "LAB-SAP-001",
                          'tags': ["customer:Stackstate", "instance:http", 'integration-type:sap', 'integration-url:LAB-SAP-001'],
                          "domain": "sap",
...
def test_get_alerts(aggregator, instance):
    ...
    with requests_mock.mock() as m:
        ...
        aggregator.assert_event(
            ...
            tags=["customer:Stackstate", "instance:http", 'status:SAPControl-GRAY', 'instance_id:{}'.format(instance_id)]
        )
        aggregator.assert_event(
            msg_text="SAPControl-GREEN",
            count=3,
            tags=["customer:Stackstate", "instance:http", 'status:SAPControl-GREEN', 'instance_id:{}'.format(instance_id)]
        )
...
def test_collect_metrics_hana_mt(aggregator, https_instance):
    ...
    with requests_mock.mock() as m:
        ...
        aggregator.assert_event(
            ...
            tags=["customer:Stackstate", "foo:bar", "status:Data backup does not exist. Without a data backup, your database cannot be recovered.", "database:ANT@ANT"]
        )
        aggregator.assert_event(
            msg_text="OK",
            count=2,
        ...
        aggregator.assert_event(
            msg_text="Data backup does not exist. Without a data backup, your database cannot be recovered.",
            tags=["customer:Stackstate", "foo:bar", "status:Data backup does not exist. Without a data backup, your database cannot be recovered.", "database:SYSTEMDB@ANT"]
        )
        aggregator.assert_event(
            msg_text="OK",
            count=2,
