    ...
    def process_all(self, filter=None):
        ...
        for rest_apis_page in self.client.get_paginator("get_rest_apis").paginate():
            for rest_api in rest_apis_page.get("items") or []:
                ...
                rest_api_data = {"RestApiId": rest_api["id"], "RestApiName": rest_api["name"]}
                ...
                stages = [stage for stage in self.client.get_stages(restApiId=rest_api_id)["item"]]
                resources = [
                    resource
                    for rest_api_resource_page in self.client.get_paginator("get_resources").paginate(
                    ...
                    resource["id"]: [
                        ...
                        for method_details in [
                            self.client.get_method(
                                restApiId=rest_api_id, resourceId=resource["id"], httpMethod=http_method
                            )
                            for http_method in sorted(resource["resourceMethods"].keys())
                        ]
                        if method_details.get("methodIntegration")
                        and method_details["methodIntegration"]["type"] in ["AWS_PROXY", "AWS", "HTTP_PROXY"]
                    ]
                ...
                for stage in stages:
                    ...
                    stage_data = {"DeploymentId": stage["deploymentId"], "StageName": stage_name}
                    if "tags" in stage:
                    ...
                    for resource in resources:
                        ...
                        for method_raw in http_methods_per_resource[resource["id"]]:
                            ...
                            method_integration_uri = (
                                replace_stage_variables(method_data["methodIntegration"]["uri"], stage["variables"])
                                if "variables" in stage
                                else method_data["methodIntegration"]["uri"]
                            )
                            ...
                            if method_data["methodIntegration"]["type"] == "HTTP_PROXY":
                                ...
                                self.emit_component(
                                    service_integration_urn, "aws.apigateway.method.http.integration", {}
                                )
