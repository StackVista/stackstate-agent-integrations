# StackState Checks Dev

# TODO: Currently the stackstate checks dev is not yet published to pypi
...
`stackstate-checks-dev` is distributed on [PyPI](https://pypi.org) as a universal wheel
...
At this point there should be a working executable, `stsdev`, in your PATH. The
help flag shortcut `-h` is available globally.

### Usage

Upon the first invocation, `stsdev` will attempt to create the config file if it
...
```console
$ stsdev
Usage: stsdev [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev clean -h
Usage: stsdev clean [OPTIONS] [CHECK]
...
```console
$ stsdev config
Usage: stsdev config [OPTIONS] COMMAND [ARGS]...
...
```toml
repo = "~/stackstate/stackstate-agent-integrations"
...
```console
$ stsdev config find -h
Usage: stsdev config find [OPTIONS]
...
```console
$ stsdev config restore -h
Usage: stsdev config restore [OPTIONS]
...
```console
$ stsdev config set -h
Usage: stsdev config set [OPTIONS] KEY [VALUE]
...
```console
$ stsdev config show -h
Usage: stsdev config show [OPTIONS]
...
```console
$ stsdev config update -h
Usage: stsdev config update [OPTIONS]
...
```console
$ stsdev create -h
Usage: stsdev create [OPTIONS] NAME
...
```console
$ stsdev dep
Usage: stsdev dep [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev dep freeze -h
Usage: stsdev dep freeze [OPTIONS]
...
```console
$ stsdev dep pin -h
Usage: stsdev dep pin [OPTIONS] PACKAGE VERSION [CHECKS]...
...
```console
$ stsdev dep resolve -h
Usage: stsdev dep resolve [OPTIONS] CHECKS...
...
```console
$ stsdev dep verify -h
Usage: stsdev dep verify [OPTIONS]
...
```console
$ stsdev env
Usage: stsdev env [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev env check -h
Usage: stsdev env check [OPTIONS] CHECK [ENV]
...
```console
$ stsdev env ls -h
Usage: stsdev env ls [OPTIONS] [CHECKS]...
...
```console
$ stsdev env prune -h
Usage: stsdev env prune [OPTIONS]
...
```console
$ stsdev env start -h
Usage: stsdev env start [OPTIONS] CHECK ENV

  Start an environment.

Options:
  -a, --agent TEXT  The agent build to use
...
```console
$ stsdev env stop -h
Usage: stsdev env stop [OPTIONS] CHECK [ENV]
...
```console
$ stsdev manifest
Usage: stsdev manifest [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev manifest set -h
Usage: stsdev manifest set [OPTIONS] KEY VALUE
...
```console
$ stsdev manifest verify -h
Usage: stsdev manifest verify [OPTIONS]
...
```console
$ stsdev metadata -h
Usage: stsdev metadata [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev metadata verify -h
Usage: stsdev metadata verify [OPTIONS] [CHECK]
...
```console
$ stsdev release
Usage: stsdev release [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev release changelog -h
Usage: stsdev release changelog [OPTIONS] CHECK VERSION [OLD_VERSION]
...
```console
$ stsdev release freeze -h
Usage: stsdev release freeze [OPTIONS]
...
```console
$ stsdev release make -h
Usage: stsdev release make [OPTIONS] CHECK [VERSION]
...
```console
$ stsdev release show
Usage: stsdev release show [OPTIONS] COMMAND [ARGS]...
...
```console
$ stsdev release show changes -h
Usage: stsdev release show changes [OPTIONS] CHECK
...
```console
$ stsdev release show ready -h
Usage: stsdev release show ready [OPTIONS]
...
```console
$ stsdev release tag -h
Usage: stsdev release tag [OPTIONS] CHECK [VERSION]
...
```console
$ stsdev release testable -h
Usage: stsdev release testable [OPTIONS]

  Create a Trello card for each change that needs to be tested for the next
  release. Run via `stsdev -x release testable` to force the use of the
  current directory.
...
```console
$ stsdev release upload -h
Usage: stsdev release upload [OPTIONS] CHECK
...
```console
$ stsdev test -h
Usage: stsdev test [OPTIONS] [CHECKS]...
...
```console
$ pip install stackstate-checks-dev
```
...
```python
from stackstate_checks.vault import Vault
...
Utilities live under the `stackstate_checks.dev` namespace and can be found [here][3].
...
```python
>>> from stackstate_checks.dev import run_command
>>>
...
```python
>>> import os
>>> from stackstate_checks.dev import temp_chdir
>>>
>>> origin = os.getcwd()
>>>
...
[2]: https://github.com/StackVista/stackstate-agent-integrations/blob/master/stackstate_checks_base/stackstate_checks/stubs/aggregator.py
[3]: https://github.com/StackVista/stackstate-agent-integrations/tree/master/stackstate_checks_dev/stackstate_checks/dev
