...
def get_current_agent_version():
    release_data = requests.get(
        'https://raw.githubusercontent.com/StackVista/stackstate-agent/master/release.json'
    ).json()
    versions = set()
...
def get_version_file(check_name):
    if check_name == 'stackstate_checks_base':
        return os.path.join(get_root(), check_name, 'stackstate_checks', 'base', '__about__.py')
    elif check_name == 'stackstate_checks_dev':
        return os.path.join(get_root(), check_name, 'stackstate_checks', 'dev', '__about__.py')
    else:
        return os.path.join(get_root(), check_name, 'stackstate_checks', check_name, '__about__.py')
...
def get_config_files(check_name):
    ...
    auto_conf = os.path.join(root, check_name, 'stackstate_checks', check_name, 'data', 'auto_conf.yaml')
    if file_exists(auto_conf):
        files.append(auto_conf)

    default_yaml = os.path.join(root, check_name, 'stackstate_checks', check_name, 'data', 'conf.yaml.default')
    if file_exists(default_yaml):
        files.append(default_yaml)

    example_yaml = os.path.join(root, check_name, 'stackstate_checks', check_name, 'data', 'conf.yaml.example')
