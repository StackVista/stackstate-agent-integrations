...
STACKSTATE_PACKAGE_PREFIX = 'stackstate-'
...
def get_package_name(folder_name):
    ...
    """
    if folder_name == 'stackstate_checks_base':
        return 'stackstate-checks-base'

    return '{}{}'.format(STACKSTATE_PACKAGE_PREFIX, folder_name.replace('_', '-'))
...
def get_folder_name(package_name):
    ...
    """
    if package_name == 'stackstate-checks-base':
        return 'stackstate_checks_base'

    return package_name.replace('-', '_')[len(STACKSTATE_PACKAGE_PREFIX):]
...
def get_agent_requirement_line(check, version):
    ...
    if check == 'stackstate_checks_base':
