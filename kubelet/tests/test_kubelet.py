...
def test_prometheus_cpu_summed(monkeypatch, aggregator, tagger):
    ...
    calls = [
        ...
        mock.call('kubernetes.cpu.usage.total', 7.756358313, ['pod_name:demo-app-success-c485bc67b-klj45', 'cluster_name:stubbed-cluster-name']),
    ]
...
def test_prometheus_net_summed(monkeypatch, aggregator, tagger):
    ...
    #
    calls = [
        mock.call('kubernetes.network.rx_bytes', 35276103554.0, ['pod_name:dd-agent-q6hpw', 'cluster_name:stubbed-cluster-name', 'interface:eth0']),
        mock.call('kubernetes.network.rx_bytes', 58107648.0, ['pod_name:fluentd-gcp-v2.0.10-9q9t4', 'cluster_name:stubbed-cluster-name', 'interface:eth0']),
    ]
    check.rate.assert_has_calls(calls, any_order=True)

    bad_calls = [
        # Make sure the per-interface metrics are not submitted
        mock.call('kubernetes.network.rx_bytes', 12638051777.0, ['pod_name:dd-agent-q6hpw', 'cluster_name:stubbed-cluster-name']),
        mock.call('kubernetes.network.rx_bytes', 22638051777.0, ['pod_name:dd-agent-q6hpw', 'cluster_name:stubbed-cluster-name']),
        # Make sure hostNetwork pod metrics are not submitted, test with and without sum to be sure
        mock.call(
            'kubernetes.network.rx_bytes',
            (4917138204.0 + 698882782.0),
            ['pod_name:kube-proxy-gke-haissam-default-pool-be5066f1-wnvn', 'cluster_name:stubbed-cluster-name'],
        ),
        mock.call(
            'kubernetes.network.rx_bytes', 4917138204.0, ['pod_name:kube-proxy-gke-haissam-default-pool-be5066f1-wnvn', 'cluster_name:stubbed-cluster-name']
        ),
        mock.call(
            'kubernetes.network.rx_bytes', 698882782.0, ['pod_name:kube-proxy-gke-haissam-default-pool-be5066f1-wnvn', 'cluster_name:stubbed-cluster-name']
        ),
    ]
    for c in bad_calls:
        assert c not in check.rate.mock_calls
...
def test_prometheus_filtering(monkeypatch, aggregator):
    ...
    with mock.patch(method_name) as mock_method:
        ...
        for name, labels, *_ in metric.samples:  # [sts] tolerate prometheus-client Sample tuple growth (0.19 had 5 fields, 0.24 has 6 with native_histogram)
